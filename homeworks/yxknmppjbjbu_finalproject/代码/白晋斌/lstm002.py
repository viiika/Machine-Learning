...
def lstm():
    ...
    for name, group in df6.groupby('id_road'):
        ...
        for i in range(6, len(group), 9):
            ...
            mydir = str('lstm/' + str(int(group.iloc[i+1][1])) + '_' + str(group.index.time[i+1]) + '_week' + str(
                group.index[i+1].weekday() + 1) + '.csv')
            X_train, y_train, X_valid, y_valid = load_data(mydir)
            ...
            mydir = str('lstm/' + str(int(group.iloc[i+2][1])) + '_' + str(group.index.time[i+2]) + '_week' + str(
                group.index[i+2].weekday() + 1) + '.csv')
            X_train, y_train, X_valid, y_valid = load_data(mydir)
